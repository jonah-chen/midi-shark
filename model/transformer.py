...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x, start_pos=0):
        ...
        _y = torch.exp(2*torch.arange(num_trigs, device=DEVICE)
                       * _y).unsqueeze(0)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layers = get_clones(EncoderLayer(
            dim_model, num_heads, dim_ff, dropout), num_layers)
...
class Transformer(nn.Module):
    ...
    def forward(self, src: Tensor, tgt: Tensor) -> Tensor:
        out = self.decoder(self.tgt_tok_emb(
            tgt), self.encoder(self.src_tok_emb(src), 3), 3)
